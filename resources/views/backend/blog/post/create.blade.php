...
@section('title', __('labels.backend.auth.users.management') . ' | ' . __('labels.backend.auth.users.create'))
                ...
                <div class="row">
                    <div class="col-sm-5">
                        <h4 class="card-title mb-0">
                            ...
                            <small class="text-muted">@lang('labels.backend.auth.users.create')</small>
                        </h4>
                    </div><!--col-->
                </div><!--row-->
...
{{--                        </div><!--form-group-->--}}
                        ...
                        <div class="form-group row">
                            {{ html()->label(__('labels.backend.auth.users.table.abilities'))->class('col-md-2 form-control-label') }}
                                ...
                                <div class="table-responsive">
                                    <table class="table">
                                        ...
                                        <tr>
                                            ...
                                            <th>@lang('labels.backend.auth.users.table.permissions')</th>
                                        </tr>
                                        </thead>
                                        <tbody>
                                        <tr>
                                                        ...
                                                        <div class="card">
                                                            <div class="card-header">
                                                                <div class="checkbox d-flex align-items-center">
                                                                    {{ html()->label(
                                                                            html()->checkbox('roles[]', old('roles') && in_array($role->name, old('roles')), $role->name)
                                                                                  ->class('switch-input')
                                                                                  ->id('role-'.$role->id)
                                                            ...
                                                            <div class="card-body">
                                                                @if($role->id != 1)
                                                                    ...
                                                                    @endif
                                                                @else
                                                                    @lang('labels.backend.auth.users.all_permissions')
                                                                @endif
                                                            </div>
                                                        </div><!--card-->
                                                    @endforeach
                                            ...
                                            <td>
                                                @if($permissions->count())
                                                    @foreach($permissions as $permission)
                                                        <div class="checkbox d-flex align-items-center">
                                                            {{ html()->label(
                                                                    html()->checkbox('permissions[]', old('permissions') && in_array($permission->name, old('permissions')), $permission->name)
                                                                          ->class('switch-input')
